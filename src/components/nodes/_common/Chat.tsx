...
import { MessageMenuActionType, MessageMenuActions } from './MessageMenu'
import { MessageMenuData } from './MessageMenuData'
    ...
    return (
        <Dropdown>
            ...
            <Menu>
                ...
                {MessageMenuData.filter(
                    (data) => data.visible?.(props.message) ?? true,
                ).map((data) => (
                    <MenuItem
                        onClick={() =>
                            props.onAction?.(props.message.id, data.action)
                        }
                        key={data.action}
                    >
                        {data.label}
...
const useMessageMenuActionHandler = (sid: string) => {
    ...
    const handleAction = useCallback(
        (messageId: string, action: MessageMenuActionType) => {
            ...
            switch (action) {
                case MessageMenuActions.Regenerate: {
                    dispatch({ type: 'regenerate', sid, mid: messageId })
                    break
                }
