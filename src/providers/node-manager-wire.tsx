...
import { DocumentManagerProvider } from '../hooks/DocumentManager'
...
export function NodeManaged({ children }: { children: React.ReactElement }) {
    ...
    const [idGenerator, setIdGenerator] = useState<NodeIdGenerator>(
        new NodeIdGenerator(),
    )
    /**
     * Chat session state
     ...
     */
    const [textToSpeechSessions, setTextToSpeechSessions] = useRecoilState(
        textToSpeechSessionsState,
    )
    const textToSpeechService = useMemo(
        () =>
            new TextToSpeechNodeService({
                sessionCreateHandler: (session) => {
                    setTextToSpeechSessions((sessions) =>
