...
const CustomSlider: React.FC<CustomSliderProps> = ({
    ...
    const handleSliderChange = (e: MouseEvent<HTMLDivElement>) => {
        if (e.target instanceof HTMLDivElement) {
            const newValue =
                orientation === 'vertical'
                    ? max -
                      ((value - e.target.offsetTop) / e.target.offsetHeight) *
                          (max - min)
                    : ((value - e.target.offsetLeft) / e.target.offsetWidth) *
                      (max - min)
            onChange(newValue)
        }
    }
    ...
    return (
        <div
            className={`relative ${
                orientation === 'vertical' ? 'h-24' : 'w-full'
            }`}
            style={{
        ...
        >
            ...
            <div
                className={`absolute ${
                    orientation === 'vertical' ? 'bottom-0 mb-1' : 'left-0 ml-1'
                }`}
                style={{
                    background: 'blue',
                    height:
